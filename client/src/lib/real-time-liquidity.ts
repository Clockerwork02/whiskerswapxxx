...
class RealTimeLiquidityManager {
  ...
  private startRealTimeUpdates() {
    // Update every 30 seconds to reduce noise
    ...
    try {
      ...
      for (const [symbol, address] of tokens) {
        ...
        try {
          ...
          // Silently handle API errors to reduce console noise
        }
      }
    } catch (error) {
    ...
    try {
      ...
      for (const [poolName, pool] of this.pools.entries()) {
        try {
          ...
          // Silently handle missing pairs - most pairs don't exist yet on testnet
        }
      }
    } catch (error) {
